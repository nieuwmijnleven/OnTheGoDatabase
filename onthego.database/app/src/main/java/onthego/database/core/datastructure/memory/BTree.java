...
public class BTree<T extends Comparable<? super T>> {
	...
	public boolean contains(T key) {
		return search(root, key) != NULL_PAIR;
	}
	...
	private void splitChild(Node<T> node, int index) {
        ...
        if (threshold - 1 >= 0) System.arraycopy(successor.key, threshold + 0, sibling.key, 0, threshold - 1);
		
		if (!sibling.isLeaf) {
            if (threshold >= 0) System.arraycopy(successor.child, threshold + 0, sibling.child, 0, threshold);
		}
	...
	private Node<T> merge(Node<T> parent, int index, Node<T> successor, Node<T> sibling) {
        ...
        if (sibling.n >= 0) System.arraycopy(sibling.key, 0, successor.key, successor.n + 0, sibling.n);
		
		if (!sibling.isLeaf) {
            if (sibling.n + 1 >= 0) System.arraycopy(sibling.child, 0, successor.child, successor.n + 0, sibling.n + 1);
		}
